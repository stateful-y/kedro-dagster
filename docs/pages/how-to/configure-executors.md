...
!!! warning "Do not commit `registry.password`"
    ...
    Keep the populated version of this file out of version control, for example in
    a git-ignored Kedro environment such as `conf/local/`. Alternatively,
    authenticate the Docker daemon out of band with `docker login` and omit the
    `registry` block entirely.
